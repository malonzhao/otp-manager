...
export class AuthController {
  ...
  async login(@Body() loginDto: LoginDto, @Language() language: string): Promise<TokensDto> {
  ...
  async register(@Body() registerDto: RegisterDto, @Language() language: string): Promise<TokensDto> {
  ...
  @HttpCode(HttpStatus.OK)
  async refresh(@Body() body: { refreshToken: string }, @Language() language: string): Promise<TokensDto> {
