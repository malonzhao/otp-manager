...
export class PlatformsService {
  ...
  async findById(id: string): Promise<Platform> {
    ...
    if (!platform) {
      throw new NotFoundException(this.i18nService.translate('platforms.not_found'));
    }
    return platform;
  }

  async create(createPlatformDto: CreatePlatformDto): Promise<Platform> {
    const existingPlatform = await this.platformsRepository.findByName(createPlatformDto.name);
    if (existingPlatform) {
      throw new ConflictException(this.i18nService.translate('platforms.already_exists'));
    }
  ...
  async update(id: string, updateData: Partial<CreatePlatformDto>): Promise<Platform> {
    const platform = await this.findById(id);
    
    if (updateData.name && updateData.name !== platform.name) {
      const existingPlatform = await this.platformsRepository.findByName(updateData.name);
      if (existingPlatform) {
        throw new ConflictException(this.i18nService.translate('platforms.already_exists'));
      }
    }
