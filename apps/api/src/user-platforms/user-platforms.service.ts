...
export class UserPlatformsService {
  ...
  async findAllByUserId(userId: string, page: number = 1, limit: number = 20): Promise<{ data: UserPlatformWithPlatform[], total: number }> {
    return this.userPlatformsRepository.findAllByUserId(userId, page, limit);
  }

  async findById(id: string, userId: string): Promise<UserPlatformWithPlatform> {
    const userPlatform = await this.userPlatformsRepository.findById(id);
    if (!userPlatform) {
      throw new NotFoundException(this.i18nService.translate('user_platforms.not_found'));
    }
    if (userPlatform.userId !== userId) {
      throw new ForbiddenException(this.i18nService.translate('common.forbidden'));
    }
    return userPlatform;
  }

  async create(userId: string, createUserPlatformDto: CreateUserPlatformDto): Promise<UserPlatformWithPlatform> {
    if (!userId) {
      throw new Error('UserId is required');
    }
    
    const existingBinding = await this.userPlatformsRepository.findByUserPlatformAndAccount(
      userId,
    ...
    if (existingBinding) {
      throw new ConflictException(this.i18nService.translate('user_platforms.already_exists'));
    }
  ...
  async update(id: string, userId: string, updateData: Partial<CreateUserPlatformDto>): Promise<UserPlatformWithPlatform> {
  ...
  async generateOTP(id: string, userId: string): Promise<{ token: string; expiresIn: number }> {
    await this.findById(id, userId);
