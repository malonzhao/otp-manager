...
export class UserPlatformsController {
  ...
  async findAll(
    ...
    return this.userPlatformsService.findAllByUserId(user.sub, parseInt(page), parseInt(limit));
  }
  ...
  async create(@Body() createUserPlatformDto: CreateUserPlatformDto, @GetUser() user: TokenPayload) {
    return this.userPlatformsService.create(user.sub, createUserPlatformDto);
  }

  @Put(':id')
  async update(@Param('id') id: string, @Body() updateData: Partial<CreateUserPlatformDto>, @GetUser() user: TokenPayload) {
