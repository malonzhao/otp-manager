...
describe('UsersService', () => {
  ...
  describe('update', () => {
    ...
    it('should update user successfully', async () => {
      ...
      const updatedUser = { ...mockUser, ...updateData, password: 'newHashedPassword' };
      ...
      expect(usersRepository.findById).toHaveBeenCalledWith('1');
      expect(usersRepository.findByEmail).toHaveBeenCalledWith(updateData.email);
      expect(usersRepository.findByUsername).toHaveBeenCalledWith(updateData.username);
    ...
    it('should not check email if email not changed', async () => {
      ...
      await usersService.update('1', { username: 'newuser', password: 'newpassword' });
    ...
    it('should throw ConflictException if new email exists', async () => {
      mockUsersRepository.findById.mockResolvedValue(mockUser);
      mockUsersRepository.findByEmail.mockResolvedValue({ ...mockUser, id: '2' });

      await expect(usersService.update('1', { email: 'existing@example.com' }))
        .rejects.toThrow(ConflictException);
    });

    it('should throw ConflictException if new username exists', async () => {
      mockUsersRepository.findById.mockResolvedValue(mockUser);
      mockUsersRepository.findByUsername.mockResolvedValue({ ...mockUser, id: '2' });

      await expect(usersService.update('1', { username: 'existinguser' }))
        .rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('activateUser', () => {
    it('should activate user successfully', async () => {
      mockUsersRepository.findById.mockResolvedValue(mockUser);
      mockUsersRepository.activateUser.mockResolvedValue({ ...mockUser, isActive: true });
  ...
  describe('deactivateUser', () => {
    it('should deactivate user successfully', async () => {
      mockUsersRepository.findById.mockResolvedValue(mockUser);
      mockUsersRepository.deactivateUser.mockResolvedValue({ ...mockUser, isActive: false });
